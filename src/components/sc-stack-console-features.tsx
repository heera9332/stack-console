...
export default function ScStackConsoleFeatures(props: Props) {
  const [active, setActive] = useState(0);
  const leftRef = useRef<HTMLDivElement>(null);
  const videoRefs = useRef<(HTMLVideoElement | null)[]>([]);

  // sections are indices "0", "1", ...
  const sections = useMemo(
    () => props.features.map((_, i) => String(i)),
    [props.features]
  );
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (visible) {
          const idx = Number(visible.target.getAttribute("data-feature") || "-1");
          if (!Number.isNaN(idx) && idx >= 0 && idx < props.features.length) {
            setActive(idx);
          }
        }
      },
  ...
  }, [sections, props.features.length]);
  ...
  return (
    <>
      <section
        id="sc-stack-console-feature"
        data-aos="fade-up"
        className="section px-4 md:px-6 lg:px-8 pt-14 md:pt-24 bg-light"
      >
        {/* Heading */}
              ...
              >
                <div>
                  ...
                  <p className="mt-4 text-body1 text-muted-foreground md:pr-14">
                    {(f.featureDescription || "").trim()}
                  </p>

                  {f.featureButtonLabel && (
                    <div className="mt-6">
                      <Link
                        href={f.featureButtonLink || "#"}
                ...
                <div
                  className="my-6 py-16 px-10 lg:hidden order-[-1]"
                  style={{ backgroundColor: f.videoSection.backgroundColor }}
                >
                  <VideoFrame feature={f} autoPlay muted loop playsInline preload="metadata" />
                </div>
              </article>
            ))}
          </div>
                  ...
                  {props.features.map((f, i) => (
                    ...
                    >
                      <VideoFrame
                        ...
                        preload="metadata"
                        onLoadedMetadata={() => {
                          if (i === active) videoRefs.current[i]?.play().catch(() => {});
                        }}
                      />
                    </div>
                  ))}
...
const isImageUrl = (u?: string) =>
  !!u && /\.(svg|png|jpe?g|webp|gif|avif)(\?.*)?$/i.test(u);

// forwardRef so parent can control play/pause
// eslint-disable-next-line react/display-name
const VideoFrame = forwardRef<HTMLVideoElement, VideoFrameProps>(
  ({ feature, className, ...rest }, ref) => {
    const posterRaw = feature.videoSection?.videoPoster?.node?.link || "";
    const poster = isImageUrl(posterRaw) ? posterRaw : undefined;

    return (
      <div className="relative w-full h-full">
        {/* Square framed container */}
        <div className="relative w-full h-full aspect-square rounded-xl md:rounded-2xl overflow-hidden shadow-[0px_0px_0px_1px_#000] bg-white">
          {/* Video fills the square */}
          <video
            ref={ref}
            className={`absolute inset-0 w-full h-full object-cover shadow-[0px_0px_0px_4px_#000] ${className ?? ""}`}
            src={feature.videoSection.video}
            poster={poster}
            {...rest}
          />
          <TopVideoFrame color={feature.videoSection.videoCardTopBackgroundColor} />
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <path
        ...
        stroke="#0B0D0F"
      />
      <path d="M501.375 18.625L504 16L506.625 18.625" stroke="#0B0D0F" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M504 23V16" stroke="#0B0D0F" strokeLinecap="round" strokeLinejoin="round" />
      <path
      ...
      />
      <path d="M522.5 23H533.5" stroke="#0B0D0F" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M528 17.5V28.5" stroke="#0B0D0F" strokeLinecap="round" strokeLinejoin="round" />
      <path
      ...
      <path
        ...
        strokeLinejoin="round"
      />
      <circle cx="26" cy="23" r="5.5" fill="black" fillOpacity="0.1" stroke="#0B0D0F" />
      <circle cx="44" cy="23" r="5.5" fill="black" fillOpacity="0.1" stroke="#0B0D0F" />
      <circle cx="62" cy="23" r="5.5" fill="black" fillOpacity="0.1" stroke="#0B0D0F" />
      <rect x="149" y="14" width="281" height="18" rx="5" fill="white" fillOpacity="0.5" stroke="#0B0D0F" />
    </svg>
  );
};
